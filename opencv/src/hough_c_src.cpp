...
void icvHoughSortDescent32s(int *sequence , int sum , int*data)
{
    // for(int i=0;i<=sum-1;i++)
    // {
    //     int tmp=data[sequence[0]];
    //     for(int j=i;j<=sum-1;j++)
    //     {
    //         if(data[sequence[j]]>tmp)
    //         tmp = data[sequence[j]];
    //     }
    //     data[sequence[i]]=tmp;
    // }
...
void HoughCircles_jian( InputArray _image, OutputArray _circles,
                       ...
                       int minRadius, int maxRadius )
{
    ...
    HoughCircles(_image, _circles, HOUGH_GRADIENT,1,120,100,30,0,0);
}
...
int main( int argc, char** argv )
{
    ...
    HoughCircles_jian( gray, circles, HOUGH_GRADIENT,1,120,100,30,0,0 );
